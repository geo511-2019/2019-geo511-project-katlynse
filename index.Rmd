...
8. Open the raster file and assign it to an object. The z values are actually depths(distance below sea level) rather than elevation (distance above sea level) so we need to multiply the z values by -1 to plot accurately.

```{r, echo=T, cache=TRUE}
sm <- raster("data/empanada_clip.tif")*(-1)
```

2. Before we get too far it's important to know what projection we need to use. Empanada is in UTM Zone 15N so we will assign it the proper projection.
...
```{r, message=FALSE, echo=TRUE, cache=TRUE}
#4a Download the sample collection file which contains X, Y and true coordinates from the GRUVEE website.
...
#Looks good! 
```
5. Use the conversion factors to convert the X, Y positions from the dive 4607 transcript to standard UTM. A few aspects of our table need to be corrected before we can do this.
```{r, message=F, echo=T}
#5a. Correct the column names
names(dive_4607) <- lapply(dive_4607[8, ], as.character) 

#5b. Remove the rows that do not contain observations
dive_4607 <- dive_4607[-c(1:8),] 

#5c. Add a column name to the samples column
names(dive_4607)[7]<-"Samples" 

#5d. Change the class of X and Y from factor to numeric
dive_4607[, "X"] <- as.numeric(as.character(dive_4607[, "X"])) 
dive_4607[, "Y"] <- as.numeric(as.character(dive_4607[, "Y"] ))

#5e. Convert X and Y to accurate Easting and Northing as new columns
dive_4607 <- mutate(dive_4607, Easting= X + 600078) 
dive_4607 <- mutate(dive_4607, Northing= Y + 224781)

#5f. Write this new table to a .csv file
